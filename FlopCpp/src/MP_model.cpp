...
namespace flopc {
    ...
    std::string MP_model::toString()const
    {
        //std::stringstream ss;
        std::ostream &ss=cout;
        ss<<"Problem: "<<getName()<<" has variables:\n";
        for(std::set<MP_variable* >::const_iterator vit = Variables.begin();
            vit!=Variables.end();vit++)
        {
            ss<<(*vit)->toString()<<'\n';
        }
        ss<<" Goal is to optimimze :"<<Objective.toString()<<'\n';
        ss<<" subect to:\n";
        for(std::set<MP_constraint* >::const_iterator cit=Constraints.begin();
