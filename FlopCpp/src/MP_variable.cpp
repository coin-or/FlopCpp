...
{
    MP_variable *nonConstThis = const_cast<MP_variable *>(this);
    std::stringstream ss;
    if(getName().length()==0)
        ss<<"unnamed";
    else
    ...
    /*
  if (offset >= 0) {
    ((*(nonConstThis->S1))(nonConstThis->i1)*(*(nonConstThis->S2))(nonConstThis->i2)*(*(nonConstThis->S3))(nonConstThis->i3)*(*(nonConstThis->S4))(nonConstThis->i4)*(*(nonConstThis->S5))(nonConstThis->i5)).Forall(this);
  } else {
    ss<<"No solution available!"<<endl;
  }
    */
  return ss.str();
}
